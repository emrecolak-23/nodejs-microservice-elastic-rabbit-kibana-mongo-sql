...
import { useAuthModal } from 'src/shared/hooks/useAuthModal';
...
const Header: FC<IHeader> = ({ navClass }): ReactElement => {
  ...
  return (
    <>
      ...
      {activeModal === 'forgotPassword' && <ForgotPasswordModal onClose={closeModal} onToggle={() => toggleModal('login')} />}
      {openSidebar && (
        <HeaderSideBar
          setShowLoginModal={() => openModal('login')}
          setShowRegisterModal={() => openModal('register')}
