...
const NotificationDropdown: FC<IHomeHeaderProps> = ({ setIsNotificationDropdownOpen }): ReactElement => {
  ...
  const { data, isSuccess } = useGetNotificationsByIdQuery(`${authUser?.username}`, {
  ...
  useEffect(() => {
    if (data?.notifications && isSuccess) {
      const sortedNotifications: IOrderNotifcation[] = [...data.notifications].sort((a: IOrderNotifcation, b: IOrderNotifcation) => {
    ...
    <div className="border-grey z-20 max-h-[470px] flex flex-col justify-between rounded shadow-md border border-grey bg-white">
      ...
      <div className="h-96 overflow-y-scroll">
        {notifications.length > 0 &&
          notifications.map((notification: IOrderNotifcation) => {
            return (
              <div
                onClick={() => {
                  ...
                  navigate(`/orders/${notification.orderId}/activities`);
                ...
                <div className="flex px-4">
                  <div className="flex-shrink-0 mt-1">
                    <img
                      ...
                      src={notification.senderUsername === authUser.username ? notification.receiverPicture : notification.senderPicture}
                      alt=""
                    />
                  </div>
                  <div className="w-full pl-3 pt-2">
                    ...
                    <div className="flex gap-2 text-[11px]">
                      <span className="font-normal text-[#b5b6ba]">{TimeAgo.transform(notification.createdAt)}</span>
                    </div>
                  </div>
                </div>
              </div>
