            ...
            <div key={uuidv4()} className="w-full rounded-lg border border-grey bg-white shadow">
              <div className="flex flex-col items-center pb-10 pt-5">
                <img
                  ...
                  src={seller.profilePicture || 'https://placehold.co/330x220?text=Profile+Image'}
                  alt="Profile image"
                />
                ...
                <div className="flex justify-center w-full gap-x-1 self-center h-6">
                  <div className="mt-1 w-20 gap-x-2 flex">
                    <StarRating
                      value={rating(parseInt(`${seller?.ratingSum}`) / parseInt(`${seller?.ratingsCount}`))}
                      size={14}
                      setReviewRating={() => {}}
                    />
                  </div>
                ...
                <div className="mt-4 flex space-x-3 md:mt-6">
                  <Link
                    to={`/seller-profile/${lowerCase(`${lowerCase(`${seller?.username}`)}`)}/${seller?._id}/view`}
