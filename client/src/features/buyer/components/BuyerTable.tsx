          ...
          <>
            <thead className="border-grey border-b text-xs uppercase text-gray-700 sm:[&>*:not(:first-child)]:hidden">
              ...
              >
                ...
                <th className="p-3 text-center">{type === BUYER_GIG_STATUS.CANCELLED ? 'Cancelled On' : 'Due On'}</th>
                ...
                return (
                  <tr key={order.orderId} className="border-grey mb-2 flex flex-col flex-nowrap border-b bg-white sm:mb-0 sm:table-row ">
                    ...
                    <td className="px-3 py-1 text-left lg:p-3 lg:text-center">
                      <span
                        className={cn(
                          'status rounded bg-transparent p-0 text-xs font-bold uppercase text-black sm:px-[5px] sm:py-[4px] sm:text-white',
                          lowerCase(order.status.replace(/_/g, ''))
                        )}
                      >
                        {order.status}
                      </span>
