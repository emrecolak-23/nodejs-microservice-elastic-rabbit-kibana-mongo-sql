...
import { cloneDeep, countriesList, degreeList, yearsList } from 'src/shared/utils/utils.service';
...
const EducationFields: FC<IEducationEditProps> = ({
  ...
  const onHandleUpdate = (): void => {
    if (type === 'add') {
      ...
      const itemIndex: number = sellerProfile?.education?.findIndex((value: IEducation) => value._id === selectedEducation?._id) ?? -1;
      const clonedEducation: IEducation[] = cloneDeep(sellerProfile?.education) as IEducation[];
