...
import { cloneDeep, languageLevel } from 'src/shared/utils/utils.service';
...
const LanguageFields: FC<ILanguageEditFieldsProps> = ({
  ...
  const onHandleUpdate = (): void => {
    if (type === 'add') {
      ...
      const itemIndex: number = sellerProfile.languages.findIndex((item: ILanguage) => item._id === selectedLanguage?._id);

      if (itemIndex !== -1) {
        const clonedItem: ILanguage = { level: !language ? '' : level, language: !language ? '' : language, _id: selectedLanguage?._id };
        const clonedLanguages: ILanguage[] = cloneDeep(sellerProfile.languages);
        clonedLanguages.splice(itemIndex, 1, clonedItem);
        const filtered = clonedLanguages.filter((item: ILanguage) => item.language !== '' && item.language !== undefined);
        if (setSellerProfile && setShowLanguageEditForm && filtered.length > 0) {
          ...
          console.log('You need have at least one language');
        }
      }
    }
  };
      ...
      <div className="mb-6 grid grid-cols-1 gap-y-3 px-3 md:grid-cols-2 md:gap-x-2">
        <div className="">
          <TextInput
            ...
            name="language"
            value={language}
            onChange={(e: ChangeEvent) => setLanguage((e.target as HTMLInputElement).value)}
          />
        </div>
      ...
      <div className="z-20 my-4 mt-10 flex cursor-pointer justify-center md:z-0 md:mt-0">
        <Button
          ...
          className={cn(
            ...
            {
              'cursor-not-allowed opacity-50': (level === 'Level' || !language) && type === 'add',
              'cursor-pointer': level === 'Level' && language && type !== 'add'
            }
          )}
