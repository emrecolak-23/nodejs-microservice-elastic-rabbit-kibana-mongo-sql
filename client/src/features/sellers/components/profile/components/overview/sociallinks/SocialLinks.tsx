      ...
      <ul className="mb-0 list-none pt-1.5">
        {showSocialLinkAddForm && (
          <li className="flex justify-between">
            <SocialLinksEdit
              type="add"
              setShowSocialLinksAddForm={setShowSocialLinkAddForm}
                ...
                <li key={link} className="flex justify-between mb-2">
                  {!showSocialLinkEditForm && (
                    <div className="col-span-3 ml-4 flex pb-3 text-sm md:text-base">
                      <a href="" target="_blank" className="mr-3 text-sky-500 no-underline hover:underline">
                        {link}
                      </a>
                    </div>
                  )}
                  {showSocialLinkEditForm && selectedLink === link && (
                    <SocialLinksEdit
                      type="edit"
                      setShowSocialLinksAddForm={setShowSocialLinkAddForm}
                      setShowSocialLinksEditForm={setShowSocialLinkEditForm}
        ...
        {sellerProfile.socialLinks.length === 0 && <li className="flex justify-between mb-2 ml-4">No information</li>}
      </ul>
    </div>
  );
};
