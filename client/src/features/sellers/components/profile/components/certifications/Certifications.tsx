    ...
    <div className="border-grey mt-6 border bg-white">
      ...
      <ul className="mb-0 list-none pt-1.5">
        {showCertificateAddForm && (
          <li className="flex justify-between">{<CertificateEdit type="add" setShowCertificateAddForm={setShowCertificateAddForm} />}</li>
        )}
        {!showCertificateAddForm && (
          <>
            {sellerProfile.certificates.map((certificate: ICertificate) => {
              return (
                <li key={certificate._id} className="mb-2 flex justify-between">
                  ...
                  {showCertificateEditForm && certificate.name === selectedCertificate?.name && (
                    <CertificateEdit
                      type="edit"
        ...
        {!sellerProfile.certificates.length && <li className="flex justify-between mb-2 ml-4">No information</li>}
      </ul>
    </div>
  );
};
