...
import { IOrderMessage, IOrderTableProps } from 'src/features/order/interfaces/order.interface';
...
const ManageOrdersTable: FC<IOrderTableProps> = ({ type, orders, orderTypes }): ReactElement => {
  ...
  const selectedOrder = useRef<IOrderDocument>(null);
          ...
          {orderTypes > 0 ? (
            <>
              <thead className="border-grey border-b text-xs uppercase text-gray-700 sm:[&>*:not(:first-child)]:hidden">
                ...
                >
                  <th className="p-3 text-center md:w-auto"></th>
                  <th className="p-3 text-left md:w-auto">Buyer</th>
                  <th className="p-3 text-left md:w-auto">Gig</th>
                  <th className="p-3 text-center ">{type === SELLER_GIG_STATUS.CANCELLED ? 'Cancelled On' : 'Due On'}</th>
              ...
              <tbody className="flex-1 sm:flex-none">
                {orders.map((order) => {
                  return (
                    <tr key={order.orderId} className="bg-white border-b border-grey flex flex-col flex-nowrap sm:table-row mb-2 sm:mb-0 ">
                      <td></td>
