...
export class ReviewServer {
  private log: Logger = winstonLogger(`${this.config.ELASTIC_SEARCH_URL}`, 'apiReviewServer', 'debug');
  constructor(
  ...
  private async startQueues(): Promise<void> {
    reviewChannel = (await this.queueConnection.connect()) as Channel;
  }
      ...
      const errorLog: Record<string, unknown> =
        err instanceof CustomError
          ...
          : {
              ...
              comingFrom: 'ReviewService errorHandler'
            };

      this.log.log('error', `ReviewService ${err.comingFrom}: `, errorLog);
  ...
  private async startServer(app: Application): Promise<void> {
    try {
      ...
      this.log.log('error', 'ReviewService startServer() error method: ', err);
    }
  }

  private async startHttpServer(httpServer: http.Server): Promise<void> {
    try {
      this.log.info(`Review server has started with process id of ${process.pid} on. review server has started`);
      httpServer.listen(SERVER_PORT, () => {
        this.log.info(`Review server running on port ${SERVER_PORT}`);
      });
    } catch (err) {
      this.log.log('error', 'ReviewService startHttpServer() error method: ', err);
    }
  }
}
