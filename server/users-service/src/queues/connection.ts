...
class QueueConnection {
  private log: Logger = winstonLogger(`${this.config.ELASTIC_SEARCH_URL}`, 'usersQueueConnection', 'debug');
  ...
  async connect(): Promise<Channel | undefined> {
    try {
      ...
      this.log.info('UsersService connected to queue successfully');
      this.handleCloseOnSigint();
      return this.channel;
    } catch (error) {
      this.log.log('error', 'UsersService QueueConnection.connect() method: ', error);
      return undefined;
    }
  }
