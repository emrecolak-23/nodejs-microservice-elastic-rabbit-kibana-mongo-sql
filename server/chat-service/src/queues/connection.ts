...
export class QueueConnection {
  private log: Logger = winstonLogger(`${this.config.ELASTIC_SEARCH_URL}`, 'chatQueueConnection', 'debug');
  ...
  async connect(): Promise<Channel> {
    ...
    try {
      ...
      this.log.info('ChatService connected to queue successfully');
      return this.channel;
    } catch (error) {
      this.log.log('error', 'ChatService QueueConnection.connect() method error:', error);
      throw error;
    } finally {
