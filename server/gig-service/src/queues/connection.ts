...
class QueueConnection {
  private log: Logger = winstonLogger(`${this.config.ELASTIC_SEARCH_URL}`, 'gigQueueConnection', 'debug');
  ...
  async connect(): Promise<Channel | undefined> {
    try {
      ...
      this.log.info('GigService connected to queue successfully');
      this.handleCloseOnSigint();
      return this.channel;
    } catch (error) {
      this.log.log('error', 'GigService QueueConnection.connect() method: ', error);
      return undefined;
    }
  }
